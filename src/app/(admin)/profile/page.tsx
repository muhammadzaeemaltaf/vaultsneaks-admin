...
export default function AdminProfile() {
  ...
  const handleSave = async () => {
    try {
      setUpdating(true)
      const updatedAdmin = await updateAdminInSanity({ ...users, ...formData }, selectedFile)
      if (updatedAdmin) {
        ...
        <div className="space-y-4">
            ...
            <Avatar className="h-24 w-24 border">
              <AvatarImage
                src={
                  selectedImage
                    ? selectedImage
                    : (users.profileImage ? urlFor(users.profileImage).url() : "")
                }
                alt={users.username}
              />
              <AvatarFallback>
                {users.username ? users.username.charAt(0).toUpperCase() : "AD"}
              </AvatarFallback>
            </Avatar>
            {isEditing && (
              <Button 
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input id="role" name="role" value={users?.role} disabled={true} />
            </div>
