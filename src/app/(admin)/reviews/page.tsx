...
import { ALL_REVIEWS_QUERYResult, Review } from "../../../../sanity.types";
import { client } from "@/sanity/lib/client";
import { Skeleton } from "@/components/ui/skeleton";

export default function ReviewsPage() {
  const [searchTerm, setSearchTerm] = useState("");
  const [reviews, setReviews] = useState<ALL_REVIEWS_QUERYResult | []>([]);
  ...
  const fetchReviews = async () => {
    setLoading(true);
    const reviewsData: ALL_REVIEWS_QUERYResult = await getAllReviews();
                ...
                filteredReviews.map((review) => (
                  <TableRow key={review._id}>
                    {deletingReviewId === review._id ? (
                      ...
                      <>
                        <TableCell>
                          <div className="relative w-14 h-14 border border-gray-200 rounded">
                            <Image
                              src={urlFor(review.product?.image ?? "").url() || "/placeholder.svg"}
                              alt={review.product?.productName ?? "Product Image"}
                              layout="fill"
                              objectFit="cover"
                              className="rounded"
                            />
                        ...
                        <TableCell className="font-medium">
                          {highlightText(review.reviewerName ?? "", searchTerm)}
                        </TableCell>
                        <TableCell>
                            ...
                            {review.reviewPicture
                              ...
                              .map((pic, index) => (
                                <div key={index} className="relative w-10 h-10">
                                  <Image
                                    src={
                                      urlFor(pic).url() || "/placeholder.svg"
                                    }
